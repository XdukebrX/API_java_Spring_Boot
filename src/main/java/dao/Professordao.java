...
public static String getProfessor_byname(String name) {
    

    Connection c = Conection.connection();//instancia a classe para conexao do banco de dados
    LinkedList<Professor> la = new LinkedList<Professor>();// cria um vetor semi infinito de Professors
    String sql;
   //  if (name.isEmpty()){
    //    sql = "SELECT * from teachers";
    // }else{
       sql = "SELECT * from teachers where id_teachers =1";//seta em uma String a querry a ser feita e define rotulo
    // }
     ...
     try { // tenta essa merda e se não joga o erro pro catch que nos retorna a jagonça 
       ...
       //  ps.setString(1, name); // troca o 1 rotulo por seu valor verdadeiro enviado quando o methodo foi chamado
         ...
         while (rs.next()) {// fica rodando o while enquanto houver um proximo valor no vetor
                 ...
                 Professor p = new Professor(rs.getString("name"), rs.getString("email"), rs.getInt("id_teachers"),rs.getString("cpf"));/**pega os valores relacionados a cada coluna especificada e 
                                                                                                                                                                      os insere em uma instacia da classe Professor a cada iteração do while*/
     ...
     String Professorjson = new Gson().toJson(la); // converte os dados de uma string para um JSON
     return Professorjson;// retorna o JSON pra oque tenha chamado o metodo usar
 }

 public static void postProfessor /**  indica que o methodo pode ser chamado sem instanciar a classe a que ele pertence **/ (Professor a/** indica que o methodo ira receber um objeto do tipo Professor**/)
 {
   Connection c =  Conection.connection();//chama a conexao para o banco de dados
   String sql = "INSERT INTO teachers (name, email,cpf) values (?,?,?)";/**seta em uma variavel string a querry a ser feita no
